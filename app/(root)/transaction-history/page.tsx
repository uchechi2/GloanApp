...
const TransactionHistory = async ({ searchParams: { id, page }}:SearchParamProps) => {
  ...
  const accounts = await getAccounts({ 
   ...
   const savings = await getAccounts({ 
    ...
    const currentBalance = parseInt(data?.currentBalance);
  const transactions = await get_transactions(userId);

  console.log('history', savings);
      ...
      <div className="space-y-6">
        <div className="transactions-account">
          ...
          <div className='transactions-account-balance'>
            ...
            <p className="text-24 text-center font-bold">{formatAmount(savings?.totalCurrentBalance)}</p>
          </div>
        </div>
